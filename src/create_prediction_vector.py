...
cursor=sql_conn.execute("""  
        select 	
                comb_table.curr_open_change, comb_table.prior_open_change, comb_table.prior_2_open_change,
        ...
        from (
        select 	stock_table.trans_id, stock_table.linux_date, stock_table.future_open_change, stock_table.curr_open_change, stock_table.prior_open_change, stock_table.prior_2_open_change,
        ...
        from (
        select 	g.linux_date, g.trans_id,
        ...
        inner join STOCK_PRICES_1DAY as b on a.trans_id=b.trans_id and a.ticker=b.ticker) as c
        left outer join STOCK_PRICES_1DAY as d on c.trans_id=d.trans_id+1 and c.ticker=d.ticker) as e
        left outer join STOCK_PRICES_1DAY as f on e.trans_id=f.trans_id+2 and e.ticker=f.ticker) as g
        left outer join STOCK_PRICES_1DAY as h on g.trans_id=h.trans_id+3 and g.ticker=h.ticker where g.ticker=?) as stock_table
        left outer join (
        select 	g.linux_date, g.trans_id,
                g.curr_open-g.prior_open as curr_open_change, g.prior_open-g.prior_2_open as prior_open_change, g.prior_2_open-h.ln_open_price as prior_2_open_change,
                g.curr_high-g.prior_high as curr_high_change, g.prior_high-g.prior_2_high as prior_high_change, g.prior_2_high-h.ln_high_price as prior_2_high_change,
        ...
        left outer join INDEX_PRICES_1DAY as b on a.trans_id=b.trans_id and a.ticker=b.ticker) as c
        left outer join INDEX_PRICES_1DAY as d on c.trans_id=d.trans_id+1 and c.ticker=d.ticker) as e
        left outer join INDEX_PRICES_1DAY as f on e.trans_id=f.trans_id+2 and e.ticker=f.ticker) as g
        left outer join INDEX_PRICES_1DAY as h on g.trans_id=h.trans_id+3 and g.ticker=h.ticker where g.ticker=".DJI") as dow_table
        ...
        left outer join INDEX_PRICES_1DAY as d on c.trans_id=d.trans_id+1 and c.ticker=d.ticker) as e
        left outer join INDEX_PRICES_1DAY as f on e.trans_id=f.trans_id+2 and e.ticker=f.ticker) as g
        left outer join INDEX_PRICES_1DAY as h on g.trans_id=h.trans_id+3 and g.ticker=h.ticker where g.ticker=".INX") as sp_table
        on comb_table.linux_date=sp_table.linux_date
        order by comb_table.trans_id desc limit 1;""",(ticker,))

# Save prediciton vector in CSV file
csv_output=open("/home/kirill/project/data/prediction_vector.csv", "w")
