...
def get_data(interval,ticker):
    ...
    for element in read_url[7:]:
        ...
        sql_conn.execute("INSERT INTO STOCK_PRICES_1DAY (TRANS_ID,LINUX_DATE,DATE_TIME,WEEK_DAY,TICKER,\
                         ...
                         (trans_id,linux_date,date_time,week_day,ticker,open_price,high_price,low_price,\     	     			  close_price,volume,ln_open_price,ln_high_price,ln_low_price,ln_close_price,\
			  ln_volume));
        sql_conn.commit()
        trans_id +=1
...
if __name__ == "__main__":
    try:
        # Get stock comprising S&P500 index
        ticker_list=[]
        sp500_file=open("/home/kirill/project/data/sp500-components.csv","r")
        for line in sp500_file:
            ticker=line.split(",")[0]
        ...
        for ticker in ticker_list:
            print(ticker)
            get_data(86400,ticker)
            
        # Close SQLite database     
        sql_conn.close()
        print("Stock data from Google Finance is loaded successfully in SQLite database")
